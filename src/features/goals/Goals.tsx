            ...
            return (
              <motion.div key={g.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Card hover className="p-5">
                  ...
                  <p className="text-xs text-white/40">Target {formatMoney(g.target, cur)}</p>
                  <div className="flex items-center justify-center my-4">
                    <ProgressRing value={pct} color={g.color} label={pct >= 100 ? 'Done!' : 'saved'} />
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-center text-xs">
                    <div className="rounded-lg bg-white/5 py-2"><div className="font-semibold text-goal">{formatMoney(g.saved, cur)}</div><div className="text-white/40">saved</div></div>
                    <div className="rounded-lg bg-white/5 py-2"><div className="font-semibold">{formatMoney(remaining, cur)}</div><div className="text-white/40">to go</div></div>
                  </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title="New savings goal">
      <div className="space-y-4">
        ...
        <Button className="w-full" disabled={!name || !target} onClick={() => onSave({ name, target: Number(target), saved: Number(saved) || 0, monthlyContribution: Number(contrib) || 0, icon, color })}>
          Create goal
        </Button>
      </div>
    </Modal>
...
function ContributeModal({ goal, onClose, onContribute, cur }: { goal: Goal | null; onClose: () => void; onContribute: (a: number) => void; cur: any }) {
  const [amt, setAmt] = useState('');
  return (
    <Modal open={!!goal} onClose={onClose} title={`Add to ${goal?.name ?? ''}`}>
      {goal && (
        <div className="space-y-4">
          <p className="text-sm text-white/50">Currently {formatMoney(goal.saved, cur)} of {formatMoney(goal.target, cur)}.</p>
          <div><Label>Contribution amount</Label><Input type="number" value={amt} onChange={(e) => setAmt(e.target.value)} placeholder="0.00" autoFocus /></div>
          <div className="flex gap-2">{[50, 100, 250, 500].map((v) => <button key={v} onClick={() => setAmt(String(v))} className="flex-1 rounded-lg bg-white/5 hover:bg-white/10 py-2 text-sm">{cur === 'GBP' ? '£' : cur === 'USD' ? '$' : '€'}{v}</button>)}</div>
